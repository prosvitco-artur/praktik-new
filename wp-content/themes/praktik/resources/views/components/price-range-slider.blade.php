<div class="range-slider relative" data-slider>
    ...
    <input type="range" id="{{ $nameFrom ?? ($name . '_from') }}-slider" min="{{ $min ?? 0 }}" max="{{ $max ?? 1000 }}" value="{{ $from ?? 0 }}" name="{{ $nameFrom ?? ($name . '_from') }}"
    ...
    <span class="absolute left-0 bottom-0"><span class="from-value">{{ $from ?? 0 }}</span> {{ $text }}</span>
