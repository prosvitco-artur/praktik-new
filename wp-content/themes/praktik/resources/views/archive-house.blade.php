  ...
  @if (! have_posts())
    <div class="text-center py-12">
      <x-alert type="warning">
        {!! __('Sorry, no houses were found.', 'sage') !!}
      </x-alert>
      ...
      <div class="mt-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <a href="{{ home_url('/garages/') }}" class="bg-gray-50 hover:bg-gray-100 p-4 rounded-lg text-center transition-colors">
            <div class="text-gray-600 text-2xl mb-2">🚗</div>
            <div class="text-sm font-medium text-gray-700">{{ __('Garages', 'sage') }}</div>
          </a>
        </div>
      </div>
    </div>
  @else
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      @while(have_posts()) @php(the_post())
        @includeFirst(['partials.content-house', 'partials.content'])
      @endwhile
    </div>
